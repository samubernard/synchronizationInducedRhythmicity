function y=BWnt(t,x,sc,K,L0,light,dark);
% BWNT Extended Becker-Weimann model of the circadian clock
%   Becker-Weimann et al. (2005) Biophys J
%   This is the actual ode-file of the model. 
%   To use with SCN3DBWNT.M, or use KOUPLING3.M to generate the coupling K.


NV=10; % number of variables
N=length(x)/NV; % number of cells
...
%%% Light
if dark < 0, % this means light pulse
    p0=-dark;
    duration=light;
    L = L0*((t>=p0) & (t<=p0+duration));
else
     L = L0*sin(pi*t.*(mod(t,light+dark)<light)/light);
end
